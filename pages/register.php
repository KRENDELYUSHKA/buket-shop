...
<body>
    ...
    <?php
    $query = $connect->prepare("SELECT menu FROM register");
    $query->execute();
    $query1 = $query->fetchAll(PDO::FETCH_COLUMN, 0);
    echo $query1[0];
    ?>
    <div class="pop_up_reg">
        <div class="pop_up_reg_body">
            <div class="pop_up_reg_body_wrap">
                <div class="pop_up_reg_body_wrap_entry">
                    <span>Регистрация в личном кабинете</span>
                </div>
                <form action="../vendor/signup.php" method="post">
                <?php
    $query = $connect->prepare("SELECT content FROM register");
    $query->execute();
    $query1 = $query->fetchAll(PDO::FETCH_COLUMN, 0);
    echo $query1[0];
    ?>
                    <div class="pop_up_reg_login">
                        <input class="pop_up_reg_login_button" type="submit" value="ЗАРЕГИСТРИРОВАТЬСЯ" name="login">
                        <div>
                            <p>Есть аккаунт?<a href="auth.php">Войти</a></p>
                        </div>
                        <p class="pop_up_reg_body_wrap_msg"></p>
                        <?php
                        if ($_SESSION['message_closed']) {
                            echo '<p class="pop_up_reg_body_wrap_msg_closed"> ' . $_SESSION['message_closed'] . ' </p>';
                        }
                        if ($_SESSION['message_open']) {
                            echo '<p class="pop_up_reg_body_wrap_msg_open"> ' . $_SESSION['message_open'] . ' </p>';
                        }
    ...
    <?php
    $query = $connect->prepare("SELECT footer FROM register");
    $query->execute();
    $query1 = $query->fetchAll(PDO::FETCH_COLUMN, 0);
    echo $query1[0];
    ?>
