...
if(isset($_POST['zHeader'])){
    $query=$connect->prepare("SELECT menu FROM delivery");
    $query->execute();
...
if(isset($_POST['oHeader'])){
    ...
    $query=$connect->prepare("UPDATE delivery SET menu = :new WHERE id='1'");
...
if(isset($_POST['zContent'])){
    $query=$connect->prepare("SELECT content FROM delivery");
    $query->execute();
    $query1=$query->fetchAll(PDO::FETCH_COLUMN, 0);
...
if(isset($_POST['oContent'])){
    ...
    $query=$connect->prepare("UPDATE delivery SET content = :new WHERE id='1'");
    $query->bindvalue(':new',$newContent);
    if($query->execute()>0){
        echo 'Данные обновлены';
    }
...
if(isset($_POST['zFooter'])){
    $query=$connect->prepare("SELECT footer FROM delivery");
    $query->execute();
    $query1=$query->fetchAll(PDO::FETCH_COLUMN, 0);
...
if(isset($_POST['oFooter'])){
    $newContent=$_POST['tableContent'];
    $query=$connect->prepare("UPDATE delivery SET footer = :new WHERE id='1'");
    $query->bindvalue(':new',$newContent);
    if($query->execute()>0){
        echo 'Данные обновлены';
    }
