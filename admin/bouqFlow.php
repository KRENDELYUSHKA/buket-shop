...
if(isset($_POST['zHeader'])){
    $query=$connect->prepare("SELECT menu FROM flow");
    $query->execute();
...
if(isset($_POST['oHeader'])){
    ...
    $query=$connect->prepare("UPDATE flow SET menu = :new WHERE id='1'");
...
if(isset($_POST['zContent'])){
    $query=$connect->prepare("SELECT content FROM flow");
    $query->execute();
    $query1=$query->fetchAll(PDO::FETCH_COLUMN, 0);
...
if(isset($_POST['oContent'])){
    ...
    $query=$connect->prepare("UPDATE flow SET content = :new WHERE id='1'");
    $query->bindvalue(':new',$newContent);
    if($query->execute()>0){
        echo 'Данные обновлены';
    }
...
if(isset($_POST['zFooter'])){
    $query=$connect->prepare("SELECT footer FROM flow");
    $query->execute();
    $query1=$query->fetchAll(PDO::FETCH_COLUMN, 0);
...
if(isset($_POST['oFooter'])){
    $newContent=$_POST['tableContent'];
    $query=$connect->prepare("UPDATE flow SET footer = :new WHERE id='1'");
    $query->bindvalue(':new',$newContent);
    if($query->execute()>0){
        echo 'Данные обновлены';
    }
