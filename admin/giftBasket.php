...
if(isset($_POST['oHeader'])){
    ...
    $query=$connect->prepare("UPDATE basket SET menu = :new WHERE id='1'");
...
if(isset($_POST['oContent'])){
    ...
    $query=$connect->prepare("UPDATE basket SET content = :new WHERE id='1'");
    $query->bindvalue(':new',$newContent);
    if($query->execute()>0){
        echo 'Данные обновлены';
    }
...
if(isset($_POST['oFooter'])){
    $newContent=$_POST['tableContent'];
    $query=$connect->prepare("UPDATE basket SET footer = :new WHERE id='1'");
    $query->bindvalue(':new',$newContent);
    if($query->execute()>0){
        echo 'Данные обновлены';
    }
